...
class SchemaEnrichmentSpec extends Specification with DataTables {
  ...
  def e1 =
    "SPEC NAME" || "EVENT" | "UNSTRUCT EVENT" | "EXPECTED SCHEMA" |
      "page view" !! event("page_view") ! Validated.Valid(None) ! SchemaKey(
        "com.snowplowanalytics.snowplow",
        "page_view",
        "jsonschema",
        SchemaVer.Full(1, 0, 0)
      ) |
      "ping ping" !! event("page_ping") ! Validated.Valid(None) ! SchemaKey(
        "com.snowplowanalytics.snowplow",
        "page_ping",
        "jsonschema",
        SchemaVer.Full(1, 0, 0)
      ) |
      "transaction" !! event("transaction") ! Validated.Valid(None) ! SchemaKey(
        "com.snowplowanalytics.snowplow",
        "transaction",
        "jsonschema",
        SchemaVer.Full(1, 0, 0)
      ) |
      "transaction item" !! event("transaction_item") ! Validated.Valid(None) ! SchemaKey(
        "com.snowplowanalytics.snowplow",
        "transaction_item",
        "jsonschema",
        SchemaVer.Full(1, 0, 0)
      ) |
      "struct event" !! event("struct") ! Validated.Valid(None) ! SchemaKey(
        "com.google.analytics",
        "event",
        "jsonschema",
        SchemaVer.Full(1, 0, 0)
      ) |
      "unstruct event" !! unstructEvent(signupFormSubmitted) ! Validated.Valid(
        Some(SpecHelpers.jsonStringToSDJ(signupFormSubmitted).right.get)
      ) ! SchemaKey(
  ...
  def e2 =
    "SPEC NAME" || "EVENT" |
      ...
      SchemaEnrichment.extractSchema(event, Validated.Valid(None)) must beLeft
    }
